...
class GaiaAssistant(ConversableAgent):
    ...
    def __init__(
        ...
        openai_assistant_id = id('asst_afLs5OX7t4egoMNTJ0k4jRhK')
        if openai_assistant_id is None:
            ...
            if len(candidate_assistants) == 0:
                ...
                if len(candidate_assistants) > 1:
                    logger.warning(
                        "Multiple assistants with name %s found. Using the first assistant in the list. "
                        "Please specify the assistant ID in llm_config to use a specific assistant.",
                        name
                    )
        ...
        for index, annotation in enumerate(annotations):
            ...
            if file_citation := getattr(annotation, "file_citation", None):
                try:
                    ...
                    citations.append("[%d] %s: %s" % (index, cited_file.filename, file_citation.quote))
                except Exception as e:
                    logger.error("Error retrieving file citation: %s", e)
            elif file_path := getattr(annotation, "file_path", None):
                try:
                    cited_file = self._openai_client.files.retrieve(file_path.file_id)
                    citations.append("[%d] Click <here> to download %s" % (index, cited_file.filename))
                except Exception as e:
                    logger.error("Error retrieving file citation: %s", e)
